...
module.exports = function(ctx, pgp) {
  ...
  ctx.createDBUser = function(user, password) {
    return ctx.query("CREATE USER " + user + " PASSWORD \'" + password + "\';");
  };

  ctx.createDatabase = function(database, owner, template) {
    return ctx.query("CREATE DATABASE " + database + " OWNER " + owner + " TEMPLATE " + template + ";");
  };
  ...
  ctx.dropDatabase = function(database) {
    return ctx.query("DROP DATABASE " + database + ";");
  };

  ctx.dropDBUser = function(user) {
    return ctx.query("DROP ROLE " + user + ";");
  };
