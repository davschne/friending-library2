...
var createDBState = function(db, obj) {
  ...
  */
  ...
  // insert books
  .then(obj.books.reduce.bind(obj.books, function(seq, book) {
    return seq.then(insertBook.bind(null, db, book));
  }, Promise.resolve()))
  // create copies
  .then(obj.copies.reduce.bind(obj.copies, function(seq, copy, index) {
    return seq.then(insertCopy.bind(null, db, copy.book, copy.owner))
    .then(function(res) {
      // add copyid property to copy object
      obj.copies[index].copyid = res[0].copyid;
    });
  }, Promise.resolve()))
  // create book requests
  .then(obj.bookrequests.reduce.bind(obj.bookrequests, function(seq, request) {
    return seq.then(insertBookRequest.bind(null, db, request.requester, request.copy.copyid, request.request_date));
  }, Promise.resolve()))
  // create borrowing
  .then(obj.borrowing.reduce.bind(obj.borrowing, function(seq, borrowing) {
